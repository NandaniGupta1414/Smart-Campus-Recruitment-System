...
//         {/* Header - Scaled Down */}
//         <div className="flex justify-between items-center mb-6 bg-white p-5 rounded-3xl shadow-sm border border-slate-200">
//           <h1 className="text-2xl font-black text-black uppercase italic">
//             Student <span className="text-blue-700">Portal</span>
//           </h1>
//           <div className="flex gap-2">
//             <button onClick={() => navigate("/student/profile")} className="bg-blue-700 text-white px-5 py-2 rounded-xl font-black text-[9px] uppercase shadow-md transition-all">Edit Profile</button>
//             <button onClick={() => {localStorage.clear(); navigate("/");}} className="bg-black text-white px-5 py-2 rounded-xl font-black text-[9px] shadow-md">Logout</button>
//           </div>
//         </div>

//         {/* 🛡️ COMPACT MASTER PROFILE CARD */}
//         <div className="bg-white rounded-[2rem] shadow-xl border-2 border-slate-200 overflow-hidden mb-8">
//           <div className="p-6 md:p-8">
            
//             {/* Identity & Education Row - All in one grid to save space */}
//             <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6 border-b border-slate-100 pb-6">
//                <div className="lg:col-span-4 border-l-4 border-blue-700 pl-3 mb-2">
//                   <h2 className="font-black text-black uppercase text-xs tracking-widest italic">Personal & Academic Info</h2>
//                </div>
//                <div><p className="text-[9px] font-black text-blue-700 uppercase italic">Name</p><p className="font-black text-sm text-black uppercase">{user?.name}</p></div>
//                <div><p className="text-[9px] font-black text-blue-700 uppercase italic">Email</p><p className="font-black text-sm text-black">{user?.email}</p></div>
//                <div><p className="text-[9px] font-black text-blue-700 uppercase italic">Phone</p><p className="font-black text-sm text-black">{user?.phone || "N/A"}</p></div>
//                <div><p className="text-[9px] font-black text-blue-700 uppercase italic">College ID</p><p className="font-black text-sm text-black underline">{user?.collegeId || "N/A"}</p></div>
               
//                {/* Academic Mini-Cards */}
//                <div className="bg-slate-50 p-3 rounded-xl border border-blue-100">
//                   <p className="text-[8px] font-black text-blue-700 uppercase italic">10th: {user?.xthBoard}</p>
//                   <p className="font-black text-lg text-black">{user?.xthMarks}%</p>
//                </div>
//                <div className="bg-slate-50 p-3 rounded-xl border border-blue-100">
//                   <p className="text-[8px] font-black text-blue-700 uppercase italic">12th: {user?.xiithBoard}</p>
//                   <p className="font-black text-lg text-black">{user?.xiithMarks}%</p>
//                </div>
//                <div className="bg-slate-50 p-3 rounded-xl border border-slate-200">
//                   <p className="text-[8px] font-black text-blue-700 uppercase italic">Course: {user?.course}</p>
//                   <p className="font-black text-lg text-black">{user?.gradMarks}%</p>
//                </div>
//                <div className="flex flex-col justify-center">
//                   <p className="text-[8px] font-black text-slate-500 uppercase italic">University / Specialization</p>
//                   <p className="font-black text-[10px] text-black uppercase">{user?.university} | {user?.branch || user?.specialization}</p>
//                </div>
//             </div>

//             {/* Skills & Project Section - Inline Grid */}
//             <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-6">
//                {/* Technical Skills */}
//                <div>
//                   <h2 className="font-black text-black uppercase text-[10px] tracking-widest mb-3 border-l-4 border-blue-700 pl-2 italic">Tech Stack</h2>
//                   <div className="flex flex-wrap gap-1.5">
//                     {user?.skills ? (
//                       (Array.isArray(user.skills) ? user.skills : user.skills.split(',')).map((skill, i) => (
//                         <span key={i} className="bg-black text-white px-3 py-1 rounded-lg text-[9px] font-black uppercase border-b-2 border-blue-700 shadow-sm">#{skill.trim()}</span>
//                       ))
...
//                <div>
//                   <h2 className="font-black text-black uppercase text-[10px] tracking-widest mb-3 border-l-4 border-slate-800 pl-2 italic">Major Project</h2>
//                   <div className="bg-blue-50 p-4 rounded-xl italic text-black text-[11px] font-bold border border-dashed border-blue-200">
//                      "{user?.projectDescription || "No project description."}"
//                   </div>
//                </div>
//             </div>

//             {/* Links & Resume - Bottom Row */}
//             <div className="flex flex-wrap items-center justify-between gap-4 pt-4 border-t border-slate-100">
//                <div className="flex gap-3">
//                   {formatURL(user?.linkedinLink) && (
//                     <a href={formatURL(user.linkedinLink)} target="_blank" rel="noopener noreferrer" className="bg-[#0077b5] text-white px-4 py-1.5 rounded-lg font-black text-[9px] uppercase hover:opacity-80 transition-all">LinkedIn</a>
//                   )}
//                   {formatURL(user?.githubLink) && (
//                     <a href={formatURL(user.githubLink)} target="_blank" rel="noopener noreferrer" className="bg-black text-white px-4 py-1.5 rounded-lg font-black text-[9px] uppercase hover:opacity-80 transition-all">GitHub</a>
//                   )}
//                </div>
//                <div className={`px-5 py-2 rounded-full font-black text-[9px] border-2 ${user?.resume ? 'bg-green-50 border-green-600 text-green-700' : 'bg-red-50 border-red-600 text-red-700'}`}>
//                   {user?.resume ? "RESUME VERIFIED ✅" : "RESUME MISSING ❌"}
//                </div>
//             </div>
...
//         {/* 🏢 JOBS SECTION */}
//         <div className="mb-10">
//           <h2 className="text-2xl font-black text-black mb-6 uppercase italic"><span className="w-10 h-2 bg-blue-700 rounded-full inline-block mr-2"></span> Open Positions</h2>
//           <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
//             {jobs.filter(job => !dismissedJobs.includes(job._id)).map((job) => {
//               const application = appliedJobsData.find(app => app.job?._id === job._id || app.job === job._id);
//               const isApplied = !!application;
//               return (
//                 <div key={job._id} className="bg-white p-5 rounded-[2rem] shadow-lg border-2 border-slate-100 relative group transition-all hover:border-blue-700 flex flex-col justify-between">
//                   <button onClick={() => handleDismiss(job._id)} className="absolute top-4 right-4 text-slate-300 hover:text-red-600 font-black text-base z-20">✕</button>
//                   {isApplied && <div className="absolute top-0 left-0 right-0 py-1 text-center text-[8px] font-black uppercase bg-blue-700 text-white rounded-t-[2rem]">Application Sent</div>}
                  
//                   <div>
//                     <div className="flex justify-between items-center mb-3 mt-2">
//                       <div className="w-10 h-10 bg-black text-white rounded-xl flex items-center justify-center text-xl font-bold">🏢</div>
//                       <div className="text-right"><p className="text-[9px] font-black text-green-700">💰 {job.salary}</p><p className="text-[9px] font-black text-slate-500 uppercase">{job.location}</p></div>
//                     </div>
//                     <h3 className="text-base font-black text-black uppercase italic leading-tight mb-1 truncate">{job.title}</h3>
//                     <p className="text-blue-700 font-black text-[9px] mb-3 uppercase tracking-wider">{job.companyName}</p>
//                     <div className="bg-slate-50 p-3 rounded-xl border border-slate-200 mb-4 h-20 overflow-y-auto"><p className="text-[10px] text-slate-700 font-bold leading-tight">{job.description}</p></div>
//                   </div>

//                   <button disabled={isApplied} onClick={() => handleApply(job._id)} className={`w-full py-3 rounded-xl font-black text-[9px] uppercase shadow-md transition-all ${isApplied ? "bg-slate-100 text-slate-400" : "bg-black text-white hover:bg-blue-700 border-b-4 border-blue-900"}`}>
//                     {isApplied ? "Apply Success ✅" : "Apply 🚀"}
//                   </button>
//                 </div>
//               );
//             })}
